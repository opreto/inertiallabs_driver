...
UDDParser::UDDParser()
  : KA(4000), KG(10), code(0), payloadLen(0), payloadInd(0), dataSet(""), high_precision_heave(false)
{
}
...
int UDDParser::parse()
{
  ...
  switch (code)
  {
    ...
    case IL_OPVT2AHR:
      ...
      dataSet =
          "\x07\x21\x23\x24\x53\x50\x52\x11\x12\x31\x32\x01\x36\x3B"
          "\x3D\x3A\xF2\xF1\xF7\x25\x41";
      break;
    case MRU_OPVTHSSHR:
      statusStream << "OPVTHSSHR";
      dataSet =
          "\x08\x21\x23\x24\x53\x50\x52\x11\x12\x13\x16\x15\x18\x31"
    ...
    case IL_OPVTAD:
      statusStream << "OPVTAD";
      dataSet =
          "\x07\x21\x23\x24\x53\x50\x52\x11\x12\x31\x32\x01\x36\x3B\xF0\x3A\xF2\xF1\x25\x41\x60\x61\x62\x63\x64\x65";
      break;
    case IL_OPVT_rawIMU:
  ...
  for (size_t i = 0; i < dataSet.size(); ++i)
  {
    switch (static_cast<uint8_t>(dataSet[i]))
    {
      ...
      case 0xF2:
        ...
        break;
      case 0xF3:
        hdrStream << "TimeStatus\tGNSSSolStatus\tGNSSPosType\tSVtrack\tSVsol\tSVsolL1\tSVSolMulti\tExtSolStatus\tGalBD"
                     "\tGPSGlo";
        break;
      case 0xF4:
        hdrStream << "UP\tUT";
      ...
      case 0xF6:
        hdrStream
            << "pBar\tTemp\tUSW\tPosType\tGNSS_ECEF_X\tGNSS_ECEF_Y\tGNSS_ECEF_Z\tGNSS_PACC\tGNSS_ECEF_VX\tGNSS_ECEF_VY\tGNSS_ECEF_VZ\tGNSS_SACC\
...
\tUTC_Hour\tUTC_Minute\tUTC_Second\tUTC_DecSec\tUTC_Day\tUTC_Month\tUTC_Year\tUTC\tLatencyECEF";
      ...
      case 0xFF:
        ...
        break;
      default:
        std::cout << "Got IMU Data Type: 0x" << std::hex << static_cast<uint8_t>(dataSet[i])
                  << " that isn't being parsed by UDDParser. EXPECT ALL DATA FROM IMU TO BE GARBAGE!!!" << std::endl;
        break;
    }
    if (i < dataSet.size() - 1)
      hdrStream << "\t";
